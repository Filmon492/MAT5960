...
\subsection{Finite Volume Scheme for a General flux \texorpdfstring{$f$}{f}}
...
\subsection{Lax--Friedrichs type Scheme and Godunov type scheme}

Let the flux be defined as $f = uV$, where $V$ is the nonlocal operator given in \eqref{eq: nonlocal operator}. There are several ways to specify the nonlocal numerical fluxes. In this work, we focus on Lax--Friedrichs type scheme and Godunov type scheme; see the approaches in \cite{blandin2016} and \cite{huang2024}.\\
If we define the numerical flux in \eqref{eq: FVM of general f} as
\begin{equation}\label{eq:Lax-F numerical flux}
    ...
    \end{split}
\end{equation}
where $\alpha >0$ is the numerical viscosity, we establish a Lax--Friedrichs type scheme designed for the nonlocal model \eqref{eq:nonlocal_model}.\\
...
\end{equation*}
Moreover, if we let the numerical flux as
\begin{equation}\label{eq:Godunov numerical flux}
    \begin{split}
        F_{j+\frac{1}{2}} = u_{j}^n V_{j+1}^n
    \end{split}
\end{equation}
We obtain a Godunov type scheme.
One may ask whether the choice of numerical quadrature will result in different outcomes when implementing the schemes developed above. That is, whether the schemes converges for some numerical quadrature weights but fails to converge for others.\\
...
The algorithm for solving \eqref{eq:nonlocal_model} using for instance the The Lax--Friedrichs type scheme becomes:\\
...
We observe that for $j = 0$ and $j = K-1$, the scheme involves undefined terms such as $u_{-1}, V_{-1}, u_{K},$ and $V_{K}$. Therefore, without additional information the scheme is valid only for internal nodes $j \in \{1,\dots,K-2\}$.
...
\end{align*}
That means the complete algorithm for solving \eqref{eq:nonlocal_model} using the Lax--Friedrichs type scheme becomes:\\
\textbf{STEP 1.} Initialize $u_{j}^{0}$ by setting:
\begin{align*}
    u_{j}^{0} = \frac{1}{\Delta x}\int_{x_{j-\frac{1}{2}}}^{x_{j+\frac{1}{2}}} u_{0}(x)\,dx, \quad \forall j \in \{0,\dots,K-1\}.
\end{align*}
